...
var directiveMetadata = []DirectiveMetadata{
	...
	{Name: "json_map_value", Block: "request", Hover: "`json_map_value <jsonpath> \"<from>\" <to-expr>;`\n\nReplaces the string value at path with the mapped result when it equals `<from>`. Unmatched values pass through unchanged (same fallthrough semantics as `model_map`). Repeat the directive for multiple mappings."},
	{Name: "json_scale", Block: "request", Hover: "`json_scale <jsonpath> in_min=<f> in_max=<f> out_min=<f> out_max=<f>;`\n\nClamps the numeric value at path to `[in_min, in_max]` and maps it linearly onto `[out_min, out_max]`. Missing/non-numeric fields are left unchanged."},
